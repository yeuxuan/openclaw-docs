...
Mattermost 是一个可自托管的团队消息平台；详见官方网站
...
- `onchar` 模式仍然会响应显式 @提及。
- `channels.mattermost.requireMention` 兼容旧配置，但推荐使用 `chatmode`。
...
与 `openclaw message send` 或定时任务/Webhook 配合使用时的目标格式：
...
- 频道中无回复：确保机器人已加入频道并提及它（oncall 模式），使用触发前缀（onchar 模式），或设置 `chatmode: "onmessage"`。
