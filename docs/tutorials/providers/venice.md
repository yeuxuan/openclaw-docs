...
| 模式           | 说明                                                                                                                 | 模型                                           |
| -------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| **私密**       | 完全私密。提示/响应**永远不会被存储或记录**。临时性的。                                                               | Llama、Qwen、DeepSeek、Venice Uncensored 等    |
| **匿名化**     | 通过 Venice 代理，元数据被剥离。底层提供商（OpenAI、Anthropic）看到的是匿名化的请求。                                | Claude、GPT、Gemini、Grok、Kimi、MiniMax       |
...
- **最佳综合质量**：`venice/claude-opus-45`，适合困难任务（Opus 仍然是最强的）。
...
| **最佳综合质量**             | `claude-opus-45`                 | Opus 仍然是困难任务中最强的               |
...
| -------------------- | ------------------------------------------------------- |
| **流式传输**        | 所有模型均支持                                          |
| **函数调用**        | 大多数模型支持（请检查 API 中的 `supportsFunctionCalling`） |
